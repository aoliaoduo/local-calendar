...
END:VCALENDAR`)
  ...
  const createdCalendar = runJson('calendar', 'create', 'CLI', 'calendar', '--color', '#039be5')
  assert.equal(createdCalendar.name, 'CLI calendar')
  const renamedCalendar = runJson('calendar', 'update', createdCalendar.id, '--title', 'Renamed calendar')
  assert.equal(renamedCalendar.name, 'Renamed calendar')
  const deletedCalendar = runJson('calendar', 'delete', createdCalendar.id)
  assert.equal(deletedCalendar.ok, true)
  ...
  const rootTask = runJson('task', 'update', task.id, '--parent', 'none')
  assert.equal(rootTask.parentId, null)
  const childTask = runJson('task', 'update', task.id, '--parent', parentTask.id)
  assert.equal(childTask.parentId, parentTask.id)

  const attachmentFixture = join(dataDir, 'attachment.txt')
  writeFileSync(attachmentFixture, 'CLI attachment')
  const attachment = runJson('attachment', 'add', 'task', parentTask.id, '-i', attachmentFixture)
  const attachments = runJson('attachment', 'list', 'task', parentTask.id)
  assert.equal(attachments.some((item) => item.id === attachment.id), true)
  const deletedAttachment = runJson('attachment', 'delete', 'task', parentTask.id, attachment.id)
  assert.equal(deletedAttachment.ok, true)

  const disposable = runJson('task', 'add', 'trash candidate')
  const deletedTask = runJson('task', 'delete', disposable.id)
  assert.equal(deletedTask[0].ok, true)
  const trashItems = runJson('trash', 'list')
  const trashItem = trashItems.find((item) => item.title === 'trash candidate')
  assert.ok(trashItem)
  const restored = runJson('trash', 'restore', trashItem.id)
