...
export function createMethodTable(svc: CalendarService): MethodTable {
  ...
  register('attachments.list', false, (p) => svc.listAttachments(p.ownerKind as 'event' | 'task', p.ownerId as string))
  register('attachments.create', true, (p) => svc.createAttachment(p as never))
  register('attachments.delete', true, (p) => svc.deleteAttachment(p.id as string))
